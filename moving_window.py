...
def process_images(h5_file_path, dataset_path, start_frame, end_frame, output_dataset, window_width, window_height, x_position_init, y_position_init, object_speed):
    with h5py.File(h5_file_path, 'a') as file:
        images = file[dataset_path]
        moving_window_images = []

        x_position = x_position_init
        speed_in_pixels_per_frame = (object_speed * 1000) / (4.42 * 40000)  # 0702_Ti64-Virgin_3: 4.42 

        for frame in tqdm(range(start_frame-1, end_frame), desc="Processing images"):
            image = images[frame]
            cropped = crop_image(image, round(x_position), y_position_init, window_width, window_height)
...
x_position_init = 43  # initial x_position (center of the window) / 0702_Ti64-Virgin_3: 43
y_position_init = 253  # initial y_position (top of the window) / 0702_Ti64-Virgin_3: 253
object_speed = 750  # adjust the object speed

process_images(h5_file_path, dataset_path, start_frame, end_frame, output_dataset, window_width, window_height, x_position_init, y_position_init, object_speed)
